...
import { apiRegister, apiMe, ApiError } from "@/features/auth/api/authApi";
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { access_token } = await apiRegister({ email, password, workspace_name: workspaceName });
    ...
    <div className="flex flex-col flex-1 justify-center px-6 py-10">
      ...
      <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Start tracking media intelligence</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input id="email" type="email" placeholder="you@company.com" required value={email}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide" htmlFor="workspace-name">Workspace Name</label>
            <input id="workspace-name" type="text" placeholder="Acme Corp" required value={workspaceName}
              onChange={(e) => setWorkspaceName(e.target.value)}
              className="w-full rounded-xl border border-input bg-muted/50 px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-colors" />
          </div>

          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide" htmlFor="password">Password</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide" htmlFor="confirm-password">Confirm Password</label>
            <input id="confirm-password" type="password" placeholder="••••••••" required value={confirmPassword}
              onChange={(e) => setConfirmPassword(e.target.value)}
              className="w-full rounded-xl border border-input bg-muted/50 px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-colors" />
          </div>

          {error && (
            <p className="text-xs text-destructive bg-destructive/10 border border-destructive/20 rounded-lg px-3 py-2" role="alert">{error}</p>
          )}
