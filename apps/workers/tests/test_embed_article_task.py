...
from tasks.nlp.embed_article import REQUIRED_DIM, _embed_text
...
def test_dimension_mismatch_raises() -> None:
    ...
    with (
        patch("tasks.nlp.embed_article._embedding_model", None),
        patch("tasks.nlp.embed_article._embedding_model_id", ""),
        patch("torch.cuda.is_available", return_value=False),
        patch("sentence_transformers.SentenceTransformer", return_value=mock_model),
    ):
        with pytest.raises(ValueError, match="1024"):
            from tasks.nlp.embed_article import _get_model
