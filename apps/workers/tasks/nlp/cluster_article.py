...
def _maybe_trigger_summary(story_id: str, member_count: int) -> None:
    ...
    from tasks.llm.generate_summary import MIN_ARTICLES_FOR_SUMMARY

    if member_count == MIN_ARTICLES_FOR_SUMMARY:
        from tasks.llm.generate_recommendation import run as generate_recommendation_task
        from tasks.llm.generate_summary import run as generate_summary_task
