...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX ix_articles_headline_gin ON articles "
    ...
    op.create_table(
        "stories",
        sa.Column("id", sa.String(36), primary_key=True),
        sa.Column("primary_entity_id", sa.String(36), sa.ForeignKey("entities.id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        "story_articles",
        sa.Column("story_id", sa.String(36), sa.ForeignKey("stories.id", ondelete="CASCADE"), primary_key=True),
        sa.Column("article_id", sa.String(36), sa.ForeignKey("articles.id", ondelete="CASCADE"), primary_key=True),
    ...
    op.create_index("ix_story_recs_story_confidence", "story_recommendations", ["story_id", "confidence_score"])
