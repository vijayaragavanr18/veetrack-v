...
class EntityModel(Base):
    ...
    aliases: Mapped[list[EntityAliasModel]] = relationship(
        "EntityAliasModel", back_populates="entity", lazy="raise", cascade="all, delete-orphan"
    )
    article_entities: Mapped[list[ArticleEntityModel]] = relationship(
        "ArticleEntityModel", back_populates="entity", lazy="raise"
    )
    stories: Mapped[list[StoryModel]] = relationship(
        "StoryModel", back_populates="primary_entity", lazy="raise"
    )
    watchlists: Mapped[list[WatchlistModel]] = relationship(
        "WatchlistModel", back_populates="entity", lazy="raise"
    )
